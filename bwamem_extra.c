...
{ // the difference from mem_align1_core() is that this routine: 1) calls mem_mark_primary_se(); 2) does not modify the input sequence
	extern mem_alnreg_v mem_align1_core(const mem_opt_t *opt, const bwt_t *bwt, const bntseq_t *bns, const uint8_t *pac, int l_seq, char *seq, void *buf, int tid);
	...
	ar = mem_align1_core(opt, bwt, bns, pac, l_seq, seq, 0, 0); // FIXME: calls as if thread 0 - but unused with bwa-gasal2 so I don't care
